...
class TestVideoDetection(unittest.TestCase):
    ...
    @patch('videodetection.draw_detections')
    ...
    def test_process_video_success_and_end_of_stream(self, mock_videocapture, mock_imshow, mock_waitkey, mock_destroy, mock_draw_detections):
        """Test successful video processing until the stream ends."""
        # --- Setup Mocks ---
        # Mock VideoCapture
        mock_cap = MagicMock()
        mock_cap.isOpened.return_value = True
        # Simulate 2 good frames, then end of stream
        ...
        mock_detections = [Detection("person", 0.9, BoundingBox(1,2,3,4))]
        mock_detector.detect_from_image.return_value = mock_detections
        mock_draw_detections.return_value = mock_frame

        # Mock waitKey to not quit
        mock_waitkey.return_value = -1

        # --- Run Function ---
        videodetection.process_video(mock_detector, 'fake.mp4', 0.5)

        # --- Assertions ---
        self.assertEqual(mock_cap.read.call_count, 3, "Should read until stream ends")
        self.assertEqual(mock_detector.detect_from_image.call_count, 2, "Should predict on two good frames")
        self.assertEqual(mock_draw_detections.call_count, 2)
        self.assertEqual(mock_imshow.call_count, 2, "Should display two good frames")
        mock_imshow.assert_called_with("YOLOv8 Inference", mock_frame)
    ...
    @patch('videodetection.cv2.imshow')
    @patch('videodetection.cv2.VideoCapture')
    def test_process_video_quit_with_q(self, mock_videocapture, mock_imshow, mock_waitkey, mock_destroy, mock_draw_detections):
        ...
        # --- Run Function ---
        videodetection.process_video(mock_detector, 0, 0.5)

        # --- Assertions ---
        self.assertEqual(mock_cap.read.call_count, 2, "Should read two frames before quitting")
        self.assertEqual(mock_detector.detect_from_image.call_count, 2, "Should predict on two frames")
        mock_cap.release.assert_called_once()
        mock_destroy.assert_called_once()
    ...
    @patch('videodetection.ObjectDetector')
    def test_main_webcam_input(self, mock_ObjectDetector, mock_process_video):
        """Test main function with default webcam input."""
        mock_detector_instance = MagicMock()
        mock_ObjectDetector.return_value = mock_detector_instance
        
        test_args = ['videodetection.py', '--model', 'm.pt']
        with patch.object(sys, 'argv', test_args):
            videodetection.main()

        mock_ObjectDetector.assert_called_once_with('m.pt')
        mock_process_video.assert_called_once_with(mock_detector_instance, 0, 0.25, output_path=ANY)
    ...
    @patch('videodetection.process_video')
    @patch('videodetection.ObjectDetector')
    def test_main_video_file_input_with_output(self, mock_ObjectDetector, mock_process_video, mock_basename, mock_splitext, mock_join):
        ...
        mock_ObjectDetector.assert_called_once_with('m.pt')
        mock_process_video.assert_called_once_with(mock_detector_instance, 'video.mp4', 0.5, output_path='output/video_timestamp.mp4')
    ...
    def test_process_video_batch_mode(self, mock_makedirs, mock_videocapture, mock_writer, mock_draw_detections, mock_tqdm):
        ...
        # --- Run Function ---
        videodetection.process_video(mock_detector, 'video.mp4', 0.5, output_path='output/vid.mp4')

        # --- Assertions ---
        mock_makedirs.assert_called_once_with('output', exist_ok=True)
        self.assertEqual(mock_cap.read.call_count, 11) # 10 good frames, 1 end
        self.assertEqual(mock_detector.detect_from_image.call_count, 10)
